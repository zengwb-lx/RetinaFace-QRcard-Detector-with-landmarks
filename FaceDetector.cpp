...
void Detector::Detect(cv::Mat& bgr, std::vector<bbox>& boxes)
{
    ...
    for (int i = 0; i < anchor.size(); ++i)
    {
        if (*(ptr1+2) > _threshold)
        {
            ...
            // loc and conf
            qr_tmp1.cx = qr_tmp.cx + *ptr * 0.1 * qr_tmp.sx;
            qr_tmp1.cy = qr_tmp.cy + *(ptr+1) * 0.1 * qr_tmp.sy;
            qr_tmp1.sx = qr_tmp.sx * exp(*(ptr+2) * 0.2);
            qr_tmp1.sy = qr_tmp.sy * exp(*(ptr+3) * 0.2);

            qr_result.x1 = (qr_tmp1.cx - qr_tmp1.sx/2) * in.w;
            if (qr_result.x1<0)
                qr_result.x1 = 0;
            qr_result.y1 = (qr_tmp1.cy - qr_tmp1.sy/2) * in.h;
            if (qr_result.y1<0)
                qr_result.y1 = 0;
            qr_result.x2 = (qr_tmp1.cx + qr_tmp1.sx/2) * in.w;
            if (qr_result.x2>in.w)
                qr_result.x2 = in.w;
            qr_result.y2 = (qr_tmp1.cy + qr_tmp1.sy/2)* in.h;
            if (qr_result.y2>in.h)
                qr_result.y2 = in.h;
            qr_result.s = *(ptr1 + 2);
            qr_result.c = 1;  //1:QRcard

            // landmark
            for (int j = 0; j < 5; ++j)
            {
                qr_result.point[j]._x =( qr_tmp.cx + *(landms + (j<<1)) * 0.1 * qr_tmp.sx ) * in.w;
                qr_result.point[j]._y =( qr_tmp.cy + *(landms + (j<<1) + 1) * 0.1 * qr_tmp.sy ) * in.h;
            }

            total_box.push_back(qr_result);
        }
